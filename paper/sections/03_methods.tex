...
transformation changed any feature's values at all, applied to Spambase and
purely cosmetic: XGBoost's tree serialization format rejects the characters
\texttt{[}, \texttt{]}, and \texttt{<} in feature names, so the column
...
unchanged. The pipeline separately includes leakage-safe median imputation
and one-hot encoding for missing or non-numeric columns, fit only on each
stress condition's training rows. None of the five datasets contains a
missing value or a non-numeric feature column (South German Credit's twenty
attributes are integer-coded and are treated as numeric throughout), so
neither step alters any value for any dataset used in this study.
