...
against the fixed, clean test set:

\begin{itemize}
\item balanced accuracy
\item worst-class recall, the lower of the two per-class recalls, which
catches a collapse in one class that balanced accuracy alone can hide
\item area under the precision-recall curve (AUPRC)
\item macro-averaged F1
\item minority-class precision and recall
\item area under the ROC curve (AUROC)
\item Brier score
\item log loss
\item expected calibration error (ECE, ten equal-width bins), which
