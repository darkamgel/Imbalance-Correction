...
\node[box] (data) {Dataset\\(1 of 10 UCI\\sources)};
...
Ten binary-classification datasets were drawn from the UCI Machine Learning
...
Heart~\cite{statlogheart}, HTRU2~\cite{lyon2015htru2}, and QSAR
Biodegradation~\cite{mansouri2013qsar}, chosen so that no single result could
be attributed to one dataset's idiosyncrasies and spanning finance, security,
healthcare, signal processing, astronomy, and chemistry.
...
Ionosphere                    & 351    & 34 & No ionospheric structure \\
Haberman's Survival           & 306    & 3  & Death within 5 years \\
Statlog Heart                 & 270    & 13 & Heart disease present \\
HTRU2                         & 17{,}898 & 8 & Pulsar candidate \\
QSAR Biodegradation           & 1{,}055 & 41 & Ready biodegradable \\
\bottomrule
\end{tabular}
\end{table}

No transformation altered the semantics of any feature. The only
transformations that changed any feature's values were column renames,
purely cosmetic, applied to Spambase and QSAR Biodegradation: XGBoost's tree
serialization format rejects the characters \texttt{[}, \texttt{]}, and
\texttt{<} in feature names, so Spambase's \texttt{char\_freq\_[} (the
...
unchanged in both cases. The pipeline separately includes leakage-safe median
imputation and one-hot encoding for missing or non-numeric columns, fit only
on each stress condition's training rows. None of the ten datasets contains a
missing value or a non-numeric feature column (South German Credit's twenty
attributes and Statlog Heart's categorical clinical measures are
integer-coded and are treated as numeric throughout), so neither step alters
any value for any dataset used in this study.
...
an average of its observed and oracle variants. Blocks with fewer than ten
complete rows are excluded as too few for a meaningful omnibus test. When
...
silently dropped. Table~\ref{tab:config} records the exact software
