...
level of label noise, perform worse than applying no correction at all. How often
this happens depends strongly on both the direction of the corruption and the
dataset the correction is applied to, and no single method we test is robust to
both corruption directions. The complete results, including per-method failure
rates and break-even points, are reported in Section~\ref{sec:results}.
...
including DistPFN and DistPFN-T~\cite{lee2026distpfn}. We check their reference
implementation directly against the authors' published code before using it, and
report an implementation error we found in that process (Section~\ref{sec:methods}).
